...
const deploy = async (
  ...
  const {
    address: proxy,
    signer: signator,
    deployTransaction: { gasPrice: gas, hash: tx, chainId: chain },
  } = deployed;
  const signer = await signator.getAddress();
  const gasPrice = gas ?? "Undefined";

  console.debug(
    ` 🍅 ${chalk.hex("#00AA7F")("Deployed in TX:")} ` + chalk.hex("#6572AA")(tx)
  );
  let loops = 0;
  ...
  while (!done && ++loops <= maxLoops) {
    try {
      impl.new = await upgrades.erc1967.getImplementationAddress(proxy);
