...
task("mint", "Mint a token for a user")
  ...
  .setAction(async (args, { ethers }) => {
    ...
    console.info(
      `Minted with token id ${chalk.hex("#ff0000")(
        args.tokenId
      )} for ${chalk.hex("00ff00")(args.address)}`
    );
  });
...
task("su", "Add a superuser")
  ...
  .setAction(async (args, { ethers }) => {
    ...
    if (user.includes(".")) {
      const rpc = (config.networks?.mainnet as HttpNetworkUserConfig)?.url;
    ...
    console.log(
      ` 🍏 Setting ${user} as superuser on ${contractName} (${address})`
    );
...
task("grant", "Grant a role")
  ...
  .setAction(async (args, { ethers }) => {
    ...
    console.log(
      ` 🍏 Setting ${user} as ${role} on ${contractName} (${address})`
    );
